...
backup_dir=/var/lib/postgresql/backup/app01_psql_backup_$date_str
...
for db in $databases; do
  ...
  sudo -u postgres pg_dump -Upostgres -v $db -F p 2>> /tmp/db_backup.log > $backup_dir/$filename
...
su - root -c "rsync -avh --update /var/lib/postgresql/backup/app01_psql_backup_*/* /data/backup/postgresql/hire"
